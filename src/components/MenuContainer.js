        ...
        <p className='text-2xl font-semibold capitalize text-headingColor relative
         ...
         py-6 overflow-x-scroll scrollbar-none'>
             {categories && categories.map(category => (
                 <motion.div
                 whileTap={{scale:0.75}}
                 key={category.id} className={`group ${filter === category.urlParamName ? 'bg-cartNumBg' :'bg-card'} w-24 min-w-[94px] h-28 cursor-pointer
                 ...
                 onClick={() => setFilter(category.urlParamName)}>
                  ...
                  group-hover:bg-white flex items-center justify-center`}>
                        ...
                        <IoFastFoodSharp  className={`text-lg ${filter === category.urlParamName ? 'text-textColor' : 'text-white'}
                        group-hover:text-textColor `}/>
                  </div> 
                  <p  className={`font-semibold  text-md ${filter === category.urlParamName ? 'text-white' : 'text-textColor'} group-hover:text-white`}>{category.name}</p>
                  </motion.div> 
